...
<<<<<<< HEAD
    async Task CaptureInstantaneousObjectAsync()
    {
        ...
        try
        {
            SetStatus("Reading Capture Objects from meter...");
            var obis = new byte[] { 0x01, 0x00, 0x5E, 0x5B, 0x00, 0xFF }; // InstantaneousReadoutOBIS
            bool ok = await _dlms.ReadBlockFromMeterAsync(obis, 0x07, 0x03, 0x00, new List<byte>());
            if (!ok) SetStatus("Failed to read capture objects from meter.", true);
            else SetStatus("Capture objects read successfully.");

            InstantaneousRows.Clear();
            int idx = 1;
            foreach (var def in InstantaneousDefinitions)
            {
                InstantaneousRows.Add(new MeterReadDisplayRow
                {
                    SerialNumber = idx++,
                    ClassId = def.ClassId,
                    ObisCode = def.ObisCode,
        ...
        {
            SetStatus($"Capture error: {ex.Message}", true);
        }
        finally
        {
            IsBusy = false;
...
<<<<<<< HEAD
        ...
        try
        {
            SetStatus("Reading Instantaneous Profile (Block)...");
            var obis = new byte[] { 0x01, 0x00, 0x5E, 0x5B, 0x00, 0xFF }; // InstantaneousReadoutOBIS
            bool ok = await _dlms.ReadBlockFromMeterAsync(obis, 0x07, 0x02, 0x00, new List<byte>());
            ...
            if (ok)
            {
                ...
                if (blockData != null && blockData.Length > 0)
                {
                    ...
                    int captureCount = blockData[nByteIndex++];
                    
                    int rowIdx = 0;
                    for (int i = 0; i < captureCount && rowIdx < InstantaneousRows.Count && nByteIndex < blockData.Length; i++)
                    {
                        var datavalue = DlmsHelper.DLMSDataFormator(blockData, nByteIndex, false);
                        if (datavalue != null && datavalue.Length >= 2)
                        {
                            InstantaneousRows[rowIdx].Value = string.IsNullOrWhiteSpace(datavalue[0]) ? "--" : datavalue[0];
                            if (int.TryParse(datavalue[1], out int nextIdx)) nByteIndex = nextIdx;
                            rowIdx++;
                        }
                        else break;
                    }
                    SetStatus($"Instantaneous read completed successfully!");
                }
                else
                {
                    SetStatus("Failed to parse Instantaneous block data.", true);
                }
            }
            else
            {
                SetStatus("Failed to read Instantaneous block.", true);
            }
        }
        catch (Exception ex)
        {
            SetStatus($"Instantaneous error: {ex.Message}", true);
        }
        finally
        {
            IsBusy = false;
...
<<<<<<< HEAD
    async Task CaptureNameplateObjectAsync()
    {
        ...
        try
        {
            SetStatus("Reading Nameplate Capture Objects from meter...");
            var obis = new byte[] { 0x00, 0x00, 0x5E, 0x5B, 0x0A, 0xFF }; // NamePlateReadoutOBIS
            bool ok = await _dlms.ReadBlockFromMeterAsync(obis, 0x07, 0x03, 0x00, new List<byte>());
            if (!ok) SetStatus("Failed to read nameplate capture objects from meter.", true);
            else SetStatus("Nameplate capture objects read successfully.");

            NameplateRows.Clear();
            int idx = 1;
            foreach (var def in NameplateDefinitions)
            {
                NameplateRows.Add(new MeterReadDisplayRow
                {
                    SerialNumber = idx++,
                    ClassId = def.ClassId,
                    ObisCode = def.ObisCode,
...
<<<<<<< HEAD
        ...
        try
        {
            SetStatus("Reading Nameplate (Block)...");
            var obis = new byte[] { 0x00, 0x00, 0x5E, 0x5B, 0x0A, 0xFF }; // NamePlateReadoutOBIS
            bool ok = await _dlms.ReadBlockFromMeterAsync(obis, 0x07, 0x02, 0x00, new List<byte>());
            ...
            if (ok)
            {
                ...
                if (blockData != null && blockData.Length > 0)
                {
                    ...
                    int rowIdx = 0;
                    for (int i = 0; i < captureCount && rowIdx < NameplateRows.Count && nByteIndex < blockData.Length; i++)
                    {
                        var datavalue = DlmsHelper.DLMSDataFormator(blockData, nByteIndex, false);
                        if (datavalue != null && datavalue.Length >= 2)
                        {
                            NameplateRows[rowIdx].Value = string.IsNullOrWhiteSpace(datavalue[0]) ? "--" : datavalue[0];
                            if (int.TryParse(datavalue[1], out int nextIdx)) nByteIndex = nextIdx;
                            rowIdx++;
                        }
                        else break;
                    }
                    SetStatus($"Nameplate read completed successfully!");
                }
                else
                {
                    SetStatus("Failed to parse Nameplate block data.", true);
                }
            }
            else
            {
                SetStatus("Failed to read Nameplate block.", true);
            }
        }
        catch (Exception ex)
        {
            SetStatus($"Nameplate error: {ex.Message}", true);
        }
        finally
        {
            IsBusy = false;
...
<<<<<<< HEAD
    ...
    {
        ShowRtcPanel = true;
        SetStatus("Ready to read RTC from meter.");
    }

    [RelayCommand]
    /* async Task ReadRtcDataAsync()
     {
         ShowRtcPanel = true;
         ...
         try
         {
             _logger.Log("Action: Reading Clock (0.0.1.0.0.255)...");
             System.Diagnostics.Debug.WriteLine("Action: Reading Clock (0.0.1.0.0.255)...");
             var ok = await _dlms.ReadByteFromMeterAsync(
      DlmsHelper.ObisCode.Clock,
      0x08,
      0x02);

             System.Diagnostics.Debug.WriteLine($"RTC Read Result: {ok}");

             var rx = _serial.ReceiveBuffer;

             int year = (rx[20] << 8) | rx[21];
             int month = rx[22];
             int day = rx[23];
             int hour = rx[25];
             int minute = rx[26];
             ...
             Timestamp =
                 $"{day:D2}/{month:D2}/{year} {hour:D2}:{minute:D2}:{second:D2}";

             _logger.Log($"SUCCESS RTC: {Timestamp}");
             SetStatus($"RTC Read: {Timestamp}");
             _logger.Log($"SUCCESS RTC: {Timestamp}");
             System.Diagnostics.Debug.WriteLine($"SUCCESS RTC: {Timestamp}");
         }
         finally
         {
             IsBusy = false;
    ...
    async Task ReadRtcDataAsync()
    {
        ...
        try
        {
            _logger.Log("Action: Reading Clock (0.0.1.0.0.255)...");
            System.Diagnostics.Debug.WriteLine("Action: Reading Clock (0.0.1.0.0.255)...");
            await _dlms.ReadByteFromMeterAsync(
                DlmsHelper.ObisCode.Clock,
                0x08,
                0x02);
            ...
            int year = (rx[rtcStart] << 8) | rx[rtcStart + 1];
            int month = rx[rtcStart + 2];
            int day = rx[rtcStart + 3];
            int weekday = rx[rtcStart + 4];
            int hour = rx[rtcStart + 5];
            int minute = rx[rtcStart + 6];
            int second = rx[rtcStart + 7];
        ...
        {
            _logger.Log($"RTC ERROR {ex.Message}");
        }
        finally
        {
            IsBusy = false;
