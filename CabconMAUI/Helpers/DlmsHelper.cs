            ...
            switch(dt){
                case 0x09: case 0x0A:{
                    ...
                    if (!ascii && len == 12 && dt == 0x09)
                    {
                        if (buf[idx+2] != 0xFF)
                        {
                            int yr=(buf[idx+2]<<8)|buf[idx+3];
                            int mo=buf[idx+4];
                            int dy=buf[idx+5];
                            int hr=buf[idx+7];
                            int mn=buf[idx+8];
                            int sc=buf[idx+9];
                            return new[]{$"{dy:D2}/{mo:D2}/{yr:D4} {hr:D2}:{mn:D2}:{sc:D2}", nextIdx.ToString()};
                        }
                        else
                        {
                            return new[]{"00/00/0000 00:00:00", nextIdx.ToString()};
                        }
                    }
                    if(dt == 0x0A || ascii){string s=System.Text.Encoding.ASCII.GetString(buf,idx+2,len).Trim('\0');return new[]{s, nextIdx.ToString()};}
                    else{string h=BitConverter.ToString(buf,idx+2,len).Replace("-","");return new[]{h, nextIdx.ToString()};}
                }
                case 0x06:{if(idx+5>buf.Length)return null;uint v=(uint)((buf[idx+1]<<24)|(buf[idx+2]<<16)|(buf[idx+3]<<8)|buf[idx+4]);return new[]{v.ToString(), (idx+5).ToString()};}
                case 0x05:{if(idx+5>buf.Length)return null;int v=(buf[idx+1]<<24)|(buf[idx+2]<<16)|(buf[idx+3]<<8)|buf[idx+4];return new[]{v.ToString(), (idx+5).ToString()};}
                case 0x12:{if(idx+3>buf.Length)return null;ushort v=(ushort)((buf[idx+1]<<8)|buf[idx+2]);return new[]{v.ToString(), (idx+3).ToString()};}
                case 0x10:{if(idx+3>buf.Length)return null;short v=(short)((buf[idx+1]<<8)|buf[idx+2]);return new[]{v.ToString(), (idx+3).ToString()};}
                case 0x11:return new[]{buf[idx+1].ToString(), (idx+2).ToString()};
                case 0x0F:return new[]{((sbyte)buf[idx+1]).ToString(), (idx+2).ToString()};
                case 0x16:return new[]{buf[idx+1].ToString(), (idx+2).ToString()};
                case 0x0C:{
                    int len=buf[idx+1];
                    int nextIdx = idx + 2 + len;
                    string s=System.Text.Encoding.UTF8.GetString(buf,idx+2,len).Trim('\0');
                    return new[]{s, nextIdx.ToString()};
                }
