...
public class DlmsService : IDlmsService
{
    ...
    public async Task<bool> PhysicalLayerConnectAsync()
    {
        try
        {
            Raise($"USB Port: {_set.SerialPort}", false);

            _ser.SetSerialPortSettings(
                _set.SerialPort,
                _set.CommandBaudRate,
                "None",
                "8",
                "1",
                _set.CommandTimeOut,
                _set.IntercharacterDelay);

            bool result = _ser.OpenPort();

            Raise($"OpenPort Result: {result}", !result);
        ...
        {
            Raise("Physical Error: " + ex.Message, true);
            return false;
        }
    }
    public void PhysicalLayerDisconnect(){try{_ser.ClosePort();}catch{}}
    public async Task<bool> HDLCLayerConnectAsync()
    {
        return await Task.Run(() =>
        {
            try
            {
                Raise("Sending SNRM Request...", false);



                bool result = _hdlc.fSendSNRM(
                    _set.ServerSAP,
                    _set.ServerLowerMacAddress,
                    _set.ClientSAP,
                    _set.CosemBufferSize,
                    _set.CosemBufferSize,
                    _set.WindowSize,
                    _set.WindowSize);


                Raise($"SNRM Result: {result}", !result);

                Raise($"SNRM RX Bytes: {_ser.BufferIndex}", false);


                if (_ser.BufferIndex > 0)
                {
                    string rx = BitConverter.ToString(
                        _ser.ReceiveBuffer,
                        0,
                        _ser.BufferIndex);

                    Raise($"SNRM RX: {rx}", false);
                }
                else
                {
                    Raise("SNRM RX: No Data", true);
                }
            ...
            {
                Raise("SNRM Error: " + ex.Message, true);
                return false;
            }
        });
    }
    ...
    {
        return _cosem.BlockBuffer.ToArray();
    }
