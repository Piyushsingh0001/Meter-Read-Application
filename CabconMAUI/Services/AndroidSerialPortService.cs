...
public class AndroidSerialPortService : ISerialPortService
{
    ...
    public bool fSendDataToPort(byte[] data,int len)
    {
        ...
        try
        {
            ...
            // 1. DO NOT toggle power - probe is already powered from OpenPort
            // Debug: Check endpoint configuration
            if (_readEndpoint?.MaxPacketSize == 0)
            {
                System.Diagnostics.Debug.WriteLine("[Android] ERROR: Read endpoint MaxPacketSize is 0");
                return false;
            }
            
            // 2. Clear USB buffer before sending (flush ghost buffer)
            FlushInputBuffer();

            // 3. Send data (ensure buffer size is multiple of packet size)
            var tx = new byte[Math.Min(len, data.Length)];
            Buffer.BlockCopy(data, 0, tx, 0, tx.Length);
            
            int result = _connection.BulkTransfer(_writeEndpoint, tx, tx.Length, 1000);
            if (result < 0) 
            {
                ...
                // The connection will be reset by re-opening the port if needed
                return false;
            }
            else if (result == 0)
            {
                System.Diagnostics.Debug.WriteLine("[Android] WARNING: BulkTransfer returned 0 bytes - possible timing issue");
                // Don't treat as error, continue with reply wait
            }
            
            System.Diagnostics.Debug.WriteLine($"[Android] Sent {result} bytes, expecting reply...");

            // 4. Immediately start a high-speed poll for reply
            return WaitForReply();
#else
            // Non-Android platforms should use real serial communication
            return false;
#endif
        }
        catch{return false;}
    ...
    public async Task<bool> ConnectAsync(string portName, int baudRate)
    {
        try
        {
            ...
            if (opened)
            {
                // Configure serial hardware with proper baud rate and Even Parity
                ConfigureSerialHardware(baudRate);
            }
    ...
    private bool OpenAndroidUsbPort()
    {
        ...
        // 4. POWER THE PROBE (Set DTR/RTS once)
        SetDtrRts(true);
    ...
    bool WaitForReply()
    {
        ...
        var quietDelay = Math.Clamp(InterchatracterDelay, 150, Math.Min(1200, Math.Max(250, CommandTimeout - MeterProcessingDelayMs)));
        var lastCount = 0;
